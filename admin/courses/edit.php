...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verifyCsrf();
    foreach (['course_code', 'course_name', 'credit_hours', 'department_id', 'semester', 'teacher_id', 'status'] as $key) {
    ...
    if (!$errors) {
        $dup = $db->prepare('SELECT COUNT(*) FROM courses WHERE course_code = ? AND id != ?');
        $dup->execute([$values['course_code'], $id]);
        if ((int) $dup->fetchColumn() > 0) {
            $errors['course_code'] = 'That course code is already used by another course.';
        }
    }

    if (!$errors) {
        $stmt = $db->prepare('
            UPDATE courses SET course_code = ?, course_name = ?, credit_hours = ?, department_id = ?,
                semester = ?, teacher_id = ?, status = ?
            WHERE id = ?
        ');
        $stmt->execute([
            $values['course_code'], $values['course_name'], (float) $values['credit_hours'],
            $values['department_id'] ?: null, $values['semester'], $values['teacher_id'] ?: null, $values['status'], $id,
        ]);
        setFlash('success', 'Course "' . $values['course_name'] . '" was updated.');
        redirect('admin/courses/index.php');
    }
}
    ...
    <div class="main-content" id="main-content">
        ...
        <div class="page-content">
            <div class="page-header">
                <div>
                    <div class="breadcrumb"><a href="<?= BASE_URL ?>admin/dashboard.php">Dashboard</a> / <a href="<?= BASE_URL ?>admin/courses/index.php">Courses</a> / Edit</div>
                    <h1>Edit Course</h1>
                </div>
                    ...
                    <form method="post" action="" data-validate novalidate>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" id="course_code" name="course_code" class="form-control <?= isset($errors['course_code']) ? 'is-invalid' : '' ?>" value="<?= h($values['course_code']) ?>" required>
