...
$errors = [];
$values = [
    'student_id' => '', 'first_name' => '', 'last_name' => '', 'gender' => 'Male',
    'date_of_birth' => '', 'email' => '', 'phone' => '', 'address' => '',
    'department_id' => '', 'program' => '', 'year_level' => 1, 'semester' => '1st Semester',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $dupStmt = $db->prepare('SELECT COUNT(*) FROM students WHERE student_id = ? OR email = ?');
        $dupStmt->execute([$values['student_id'], $values['email']]);
        if ((int) $dupStmt->fetchColumn() > 0) {
            $errors['student_id'] = 'That Student ID or email is already in use.';
        }
    }
    ...
    if (!$errors && !empty($_FILES['photo']['name'])) {
        try {
            $photoPath = handlePhotoUpload($_FILES['photo'], 'students');
    ...
    if (!$errors) {
        $stmt = $db->prepare("
            INSERT INTO students
                (student_id, first_name, last_name, gender, date_of_birth, email, phone, address,
                 department_id, program, year_level, semester, admission_date, photo, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            ...
            $values['semester'], $values['admission_date'] ?: null, $photoPath, $values['status'],
        ]);

        setFlash('success', 'Student "' . $values['first_name'] . ' ' . $values['last_name'] . '" was added.');
    ...
    <div class="main-content" id="main-content">
        ...
        <div class="page-content">
            <div class="page-header">
                <div>
                    <div class="breadcrumb"><a href="<?= BASE_URL ?>admin/dashboard.php">Dashboard</a> / <a href="<?= BASE_URL ?>admin/students/index.php">Students</a> / Add</div>
                    <h1>Add Student</h1>
                </div>
                    ...
                    <form method="post" action="" enctype="multipart/form-data" data-validate novalidate>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" id="student_id" name="student_id" class="form-control <?= isset($errors['student_id']) ? 'is-invalid' : '' ?>" value="<?= h($values['student_id']) ?>" placeholder="STU-2026-001" required>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="program" name="program" class="form-control" value="<?= h($values['program']) ?>" placeholder="e.g. BSc Computer Science">
                            </div>
                        </div>
                        ...
                        <div style="display:flex;gap:12px;margin-top:8px;">
                            <button type="submit" class="btn btn-primary"><i class="fa-solid fa-check"></i> Save Student</button>
